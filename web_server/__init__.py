...
class HAFlask(Flask):
    ...
    def _register_routes(self):
        ...
        def index():
            ...
            self.__logger.debug(f'{route} state: {self.state}')
        ...
        def mqtt_toggle():
            ...
            self.state['Error'] = ''
            self.__logger.debug(f'{route} mqtt_toggle() state: {self.state} -------------------')
            self.__logger.debug(f'{route} client.is_connected(): {self.client.is_connected()}')
            if self.state['Connected'] != self.client.is_connected():
            ...
            if not self.state['Discovered'] and not self.client.is_connected():
                ...
                self.__logger.debug(f'{route} state: {self.state}, client.connect_mqtt()')
                ...
                self.__logger.debug(f'{route} state["Discovered"]: {self.state["Discovered"]}')
                if self.state['Discovered']:
                    ...
                    self.__logger.debug(f'{route} state: {self.state}, client.disconnect_mqtt()')
                    self.client.disconnect_mqtt()
                    self.state['Connected'] = False
        
            self.__logger.debug(f'{route} return jsonify(state): {self.state}')
            return jsonify(self.state)
        ...
        def discovery_toggle():
            ...
            self.__logger.debug(f'{route} return self.state: {self.state}');
            return jsonify(self.state)
