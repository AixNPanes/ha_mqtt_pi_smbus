...
MOCKED_OPEN = MagicMock(side_effect=lambda file, *args, **kwargs: (
    mock_open(read_data=MOCK_CPUINFO_DATA).return_value if file == '/proc/cpuinfo'
    else mock_open(read_data=MOCK_OSRELEASE_DATA).return_value if file == '/etc/os-release'
    else mock_open(read_data=MOCK_SYS_CLASS_THERMAL_THERMAL_ZONE0_TEMP).return_value if file == '/sys/class/thermal/thermal_zone0/temp'
