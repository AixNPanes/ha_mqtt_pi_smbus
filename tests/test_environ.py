...
from ha_mqtt_pi_smbus.environ import getTemperature, getMacAddress, getMacAddressByInterface, getObjectId, getCpuInfo, getOSInfo
...
class TestDevice(unittest.TestCase):
    ...
    def test_mac_address_eth_success(self):
        ...
        """
        with patch("ha_mqtt_pi_smbus.environ.subprocess.check_output", return_value=fake_ifconfig_output.encode("utf-8")):
    ...
    def test_mac_address_wlan_success(self):
        ...
        """
        with patch("ha_mqtt_pi_smbus.environ.subprocess.check_output", return_value=fake_ifconfig_eth0_output.encode("utf-8")):
            mac = getMacAddressByInterface("eth0")
            assert mac == "b8:27:eb:94:a7:18"
        with patch("ha_mqtt_pi_smbus.environ.subprocess.check_output", return_value=fake_ifconfig_wlan0_output.encode("utf-8")):
            mac = getMacAddressByInterface("wlan0")
            assert mac == "b8:27:eb:94:a7:19"

    def test_mac_address_fake_except(self):
        with patch("ha_mqtt_pi_smbus.environ.subprocess.check_output") as mock_check_output:
            mock_check_output.side_effect = subprocess.CalledProcessError(1, ["ifconfig", "fake0"])
    ...
    def test_mac_address_wlan0(self):
        fake_ifconfig_wlan0_output = """
            ...
            ether b8:27:eb:94:a7:19  txqueuelen 1000  (Ethernet)
        """.encode('utf-8')
        with patch("ha_mqtt_pi_smbus.environ.subprocess.check_output", side_effect=[''.encode('utf-8'),fake_ifconfig_wlan0_output]) as mock_check_output:
            mac = getMacAddress()
            assert mac == 'b8:27:eb:94:a7:19'
