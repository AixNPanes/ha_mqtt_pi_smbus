...
class TestDevice(unittest.TestCase):
    ...
    @patch(
        "ha_mqtt_pi_smbus.environ.readfile",
...
Model		: Raspberry Pi 3 Model B Rev 1.2""",
    ...
    @patch(
        "ha_mqtt_pi_smbus.environ.readfile",
        return_value='''PRETTY_NAME="Debian GNU/Linux 12 (bookworm)"
...
BUG_REPORT_URL="https://bugs.debian.org/"''',
    ...
    @patch(
        "ha_mqtt_pi_smbus.environ.get_command_data",
        return_value="up 2 days, 7 hours, 0 minutes",
    )
    ...
    @patch(
        "ha_mqtt_pi_smbus.environ.get_command_data", return_value="2020-01-01 01:02:03"
    )
    ...
    @patch(
        "ha_mqtt_pi_smbus.environ._get_pyproject_version",
    ...
    @patch(
        "ha_mqtt_pi_smbus.environ._get_metadata_version",
        side_effect=[
    ...
    def test_get_version(self):
        with patch(
            "ha_mqtt_pi_smbus.environ._get_pyproject_version", return_value="v0.1.3"
        ):
            self.assertEqual(get_version(), "v0.1.3")
        with patch(
            "ha_mqtt_pi_smbus.environ._get_setuptools_version", return_value="v0.1.4"
        ):
            self.assertEqual(get_version(), "v0.1.4")
        with patch(
            "ha_mqtt_pi_smbus.environ._get_metadata_version", return_value="v0.1.5"
        ):
            self.assertEqual(get_version(), "v0.1.5")
        with patch(
            "ha_mqtt_pi_smbus.environ._get_package_version", return_value="v0.1.6"
        ):
