...
from ha_mqtt_pi_smbus.mqtt_client import MQTTClient, State

class HAFlask(Flask):
    def __init__(self, import_name, parser, client:MQTTClient, device:HADevice, _debug_step_count:int = 20):
        route = '__init__'
    ...
    def _register_routes(self):
        ...
        def mqtt_toggle():
            route = f'{request.path} [{request.method}]'
            state = self.client.state.validate(request.get_json(), self.client.is_connected())
