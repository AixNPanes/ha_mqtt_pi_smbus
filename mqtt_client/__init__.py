...
from sensor import HASensor
from bme_280 import BME_280

class MQTT_Publisher_Thread(threading.Thread):
    def __init__(self, client:"MQTTClient", sensor: HASensor, bme280:BME_280):
    ...
    def run(self):
        while True:
            ...
            if data['last_update'] != self.data['last_update']:
                self.data = data
                self.client.publish(self.sensor.state_topic, json.dumps(data))
...
class MQTTClient(mqtt.Client):
    ...
    def __init__(self, client_prefix:str, sensor:Sensor, bme280:BME_280, mqtt_config:dict = None):
        super().__init__(
    ...
    def publish_discoveries(self, devices:dict):
        self.publisher_thread = MQTT_Publisher_Thread(self, self.sensor, self.bme280)
